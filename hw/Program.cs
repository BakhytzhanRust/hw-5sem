...
//   for (int i = 0; i < Length; i++)
//   {
//     array[i] = Math.Round(random.NextDouble() * random.Next(-500,500),2); // Решил, что это будет оптимальным на данный момент, так как задача не обозначает конкретные рамки, а делать бесконечность не умею.
//   }
//   return array;
// }
